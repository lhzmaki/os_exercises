...
class scheduler:
    ...
    def move_to_running(self, expected):
        if (expexted == STATE_READY):
            self.pro_info[self.curr_proc][PROC_STATE] = STATE_RUNNING
        return

    #change to DONE STATE, the current proc's state should be expected
    def move_to_done(self, expected):
        if (expexted == STATE_RUNNING):
            self.pro_info[self.curr_proc][PROC_STATE] = STATE_DONE
        return
    ...
    def next_proc(self, pid=-1):
        if (pid == -1):
            pid = self.curr_proc
        pid += 1
        if(pid == get_num_processes):
                pid = 0
        while (self.proc_info[pid][PROC_STATE] != STATE_READY):            
            pid += 1
            if(pid == get_num_processes):
                pid = 0
        return
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            if self.proc_info[self.curr_proc][PROC_STATE] == STATE_RUNNING and \
                ...
                instruction_to_execute = self.get_instruction(self, self.curr_proc, 0)
                self.proc_info[proc_id][PROC_CODE].pop(0)
