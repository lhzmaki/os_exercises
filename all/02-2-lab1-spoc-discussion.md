...
>  在kern_init()函数中调用idt_init()函数实现IDT的初始化（内部用SETGATE函数实现IDT项的初始化）；调用clock_init()函数实现时钟中断的中断描述符的初始化，并将8253时钟初始化为每秒打断100次；调用pic_init()函数实现8259中断控制器的初始化
...
>  以内部时钟完成IQR_timer的使用，完成了对CGA/VGA，键盘，串口的访问
...
> 为每个系统调用接口如sys_kill关联一个计数器，初始化为0，在该接口的第一行让其加一，这样该计数器的值就是该系统调用的调用次数。

> 在第一一个全局变量num，num的值为所有系统调用接口计数器值得合，则num的值就是所有系统调用次数
